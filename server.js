...
app.all('/simaish', async(req, res) => {
    try {

        const summary = req.body.operatorResults?.[1]?.result?.text;
        const conversation_sid = req.body.conversationId
        const intelligence_sid = req.body.intelligenceConfiguration.ruleId

        const parti_obj = req.body.operatorResults?.[0]?.executionDetails?.participants || [];
        ...
        async function updateTrait({ key, value }) {
            try {
                ...
                const mem_store = process.env.TWILIO_MEMORY_STORE_SID
    ...
    catch (error) { console.log("Error:", error.message); res.status(500).send(error.message) }
